...
def harrison_ford
  ...
  # appeared but not as a lead actor
    ...
    movies = Movie.select(:id, :title).joins(:actors).where(actors: { name: 'Harrison Ford' })
    movies.where.not(castings: {ord: 1})
end
...
def directed_by_one_of(them)
  ...
  # ids = Actor.where(name: them).pluck(:id)
  # Movie.select(:id, :title).where(director_id: ids)
